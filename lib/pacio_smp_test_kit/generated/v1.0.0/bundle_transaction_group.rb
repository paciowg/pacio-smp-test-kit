...
module PacioSMPTestKit
  module PacioSMPV100
    class BundleTransactionGroup < Inferno::TestGroup
      ...
      short_description 'Verify support for the server capabilities required by the Standardized Medication Profile - Bundle Medication List Maintenance.'
      description %(
  # Background
...
the [Standardized Medication Profile - Bundle Medication List Maintenance](http://hl7.org/fhir/us/smp/StructureDefinition/smp-bundle-transaction). Each element is checked against
...
read succeeds.
      ...
      def self.metadata
        @metadata ||= Generator::GroupMetadata.new(YAML.load_file(File.join(__dir__, 'bundle_transaction', 'metadata.yml'), aliases: true))
      end
  
      test from: :smp_v100_bundle_transaction_read_test
      test from: :smp_v100_bundle_transaction_validation_test
      test from: :qi_core_v100_bundle_transaction_must_support_test
    end
  end
end
